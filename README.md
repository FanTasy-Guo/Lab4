...
本次实验以经典 **Phong 局部光照模型**为核心，在 Python + Taichi 并行计算框架下，通过纯数学方式（光线投射 Ray Casting）构建三维场景，实现了从几何求交、着色计算到交互式 UI 的完整渲染管线。
...
> **总结**：对于大入射角场景，Blinn-Phong 的高光边缘更柔和，消除了 Phong 模型的"硬截边"伪影；两者在中小入射角下视觉差异较小，但 Blinn-Phong 整体高光叶形更宽，因此相同 Shininess 值下高光范围略大。
...
硬阴影（Hard Shadow）基于**暗影射线（Shadow Ray）**技术。其核心思想是：在光线与物体表面的交点处，向光源方向额外发射一条测试射线。若该测试射线在到达光源之前被场景中的其他几何体遮挡，则说明交点处于阴影之中——此时该点无法接收直接光照，仅保留**环境光**分量；反之，则执行完整的 Blinn-Phong 着色计算。
...
- **自交偏移（Epsilon Offset）**：暗影射线的起点沿表面法线方向偏移 $\varepsilon = 10^{-4}$，防止射线与当前交点所在表面发生自相交而产生"痤疮"（Acne）噪点伪影；
...
> 实际运行截图见提交仓库 README 或实验演示视频。

---

*实验代码已上传至 GitHub 仓库，详见提交链接。*
